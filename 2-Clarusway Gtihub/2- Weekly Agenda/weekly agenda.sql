...
SELECT  *FROM sales.customers XWHERE EXISTS(select distinct first_name,last_name,YEAR(order_date), COUNT(order_id) as number_of_ordersfrom sales.customers A, sales.orders Bwhere A.customer_id = B.customer_idAND X.customer_id=B.customer_idgroup by YEAR(order_date),first_name,last_namehaving COUNT(order_id) > 1);
...
select product_name,a.list_price,quantityfrom sales.order_items A, production.products Bwhere  A.order_id = B.product_idand quantity >= 2
